...
본 세션에서 직접 활용한 것은 **Redis Search** 하나(벡터 cache + agent memory 두 패턴 모두). 나머지 모듈은 같은 AMR 인스턴스 안에서 동시 활용 가능 — 예: agent 시스템에서 Search로 벡터 검색 + JSON으로 대화 state 저장 + Bloom으로 중복 prompt 검출 + TimeSeries로 응답 latency 메트릭 기록.
...
데모는 코드 페이지를 거의 노출하지 않고 동작 데모 중심이었습니다. 세션 종료 멘트에서 *"different samples of the demos on GitHub"* 만 언급되었고 정확한 데모 repo URL은 미공개. 관련 공식 리소스 링크만 정리:
...
- **Microsoft Agent Framework × AMR 네이티브 통합** — 발표 멘트만. 정확한 SDK API · 패키지 · 샘플 repo URL은 세션에서 미공개. Agent Framework 공식 docs로 cross-check 후 추가 보강 필요.
- **데모 GitHub samples repo** — Roy 종료 발언에서 언급되지만 정확한 URL 미공개. 본 노트는 Roy의 멘트와 발표자 fork 이력에 근거해 [redis-developer/redis-ai-resources](https://github.com/redis-developer/redis-ai-resources) 와 [redis-developer/langgraph-redis](https://github.com/redis-developer/langgraph-redis) 를 "관련 공식 리소스" 로만 표시.
