...
## 5. 에이전트의 build · host · govern 이 한 플랫폼 안으로 — Microsoft Foundry + Agent 365

> *"Microsoft Foundry, the platform for **building, hosting, and governing** long-running AI agents."*
>
> *"Foundry's governance layer, Agent 365, ensures **containment, security, and compliance** across all agentic systems."*  
> — Build 2026 opening keynote, KEY01

최근 1년 사이 에이전트를 직접 만드는 패턴은 **agent loop(모터) 위에 harness(차체)** 를 올리는 형태로 어느 정도 표준화됐고, Foundry 는 이 harness 를 **managed 로 운영해주는 런타임**을 제공하는 위치를 잡으려 합니다. RAG 도 더 이상 고객이 retrieval 파이프라인을 직접 운영하는 게 아니라 **Foundry 안에서 retrieval 이 동작**하고, 고객은 *지식 그 자체* 에만 집중하면 되는 모양으로 옮겨갑니다. 거버넌스는 **Agent 365** 가 챙기는데 — 이게 단순히 새 콘솔이 아니라 **M365 와 통합된 모델** 위에서, 보안팀이 *사람과 동등한 레벨* 로 에이전트를 관리하는 지점입니다. 한 줄로 보면 Foundry 의 위치를 **모델(LLM) 공급자에서 → 에이전트 통합 플랫폼** 으로 다시 그리는 발표였습니다.

**① BRK243 — Agent harness · Hosted agents.** 세션의 중심 비유는 *agent loop 는 모터, harness 는 차체* 입니다. loop 는 context 처리 → 도구 실행 → 응답 정제를 목표 달성까지 반복하는 핵심 지능이고, **harness** 는 그 바깥에서 context 압축 · 도구 접근 · 세션 간 메모리 · lifecycle hook · 위험 작업의 human-in-the-loop 를 책임집니다. 단일 프롬프트 에이전트와 long-running 에이전트를 가르는 경계가 바로 이 계층입니다. 구현 선택지는 둘 — claw 계열 **Hermes** 같은 off-the-shelf harness 를 그대로 쓰거나, **Microsoft Agent Framework** (Python · C#) 로 loop · workflow · harness 를 직접 조립합니다 (OpenAI · Anthropic · Gemini 모델 혼용 가능). Foundry 는 이 harness 운영 계층을 **Hosted agents (Public Preview)** 로 흡수합니다 — per-session VM sandbox · `$HOME`/`/files` 영속 · 15분 idle scale-to-zero + 자동 복원 · 에이전트별 전용 Entra identity. LangGraph · OpenAI Agents SDK · Anthropic Agent SDK · GitHub Copilot SDK 로 짠 코드도 컨테이너로 받아 돌립니다.
...
**② BRK246 — Foundry IQ · agentic retrieval.** **Serverless Foundry IQ(Public Preview)** 는 복잡 질의를 subquery 로 분해 → 병렬 실행 → semantic rerank → 통합하는 **agentic retrieval** 을 SaaS 로 제공합니다. Search Index · Blob 은 hybrid, MCP · Web 은 federated 로 합치고, 그 위에 **Entra 권한 · Purview 라벨이 검색 결과로 propagate** 돼 호출자의 권한을 넘는 답을 막습니다. 고객이 다룰 면은 *"어떤 지식을 어떤 정책으로 노출할지"* 로 좁아집니다.
...
**③ BRK251 — Agent 365 (거버넌스 레이어).** Agent 365 는 Foundry 의 거버넌스 레이어로 명시됐는데, 핵심은 *기존 사람·디바이스·문서를 다루던 같은 스택* — **Entra (ID) · Defender (보안) · Purview (데이터) · Intune (디바이스)** — 위에 얹힌다는 점입니다. 새로 들어온 **Agent ID** 가 사람의 user identity 와 같은 자리에 위치하고, 보안팀에게 에이전트는 *새 종류의 시스템* 이 아니라 *관리해야 할 새 종류의 actor* 가 됩니다.
...
세 동사가 한 플랫폼 위에서 실제로 어떻게 맞물리는지 — harness 구성요소별 성숙도, agentic retrieval 의 품질 개선 수치, Agent 365 의 admin 자동화는 — 각 세션 노트에.

- → [KEY01 — Foundry + Agent 365 키노트 프레임](../sessions/KEY01-microsoft-build-opening-keynote.md)
- → [BRK243 — Agent harness · Hosted agents](../sessions/BRK243-claw-agent-harness-microsoft-foundry.md)
- → [BRK246 — Agentic retrieval · Serverless Foundry IQ](../sessions/BRK246-foundry-iq-enterprise-knowledge-agentic-retrieval.md)
- → [BRK251 — Agent 365 · Agent ID · M365 거버넌스 확장](../sessions/BRK251-build-secure-enterprise-ready-agents-agent-365.md)
