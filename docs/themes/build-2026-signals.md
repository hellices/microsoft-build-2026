...
Satya 가 Build 2026 키노트에서 announce 한 **Azure HorizonDB** 는 그 다섯을 **PostgreSQL 한 엔진 안에서 처리**하겠다는 제안입니다. `SELECT azure_ai.generate(...)` 한 줄로 LLM 을 부르고, `model_registry.model_add(...)` 로 BYOM 임베딩 모델을 등록합니다. compute / storage 가 분리돼 있어 **read replica 는 데이터 복제 없이 즉시** 추가되고 — 최대 15 replicas, 발표 수치 기준 throughput 3× · vector search 3× — **2026-06 Public Preview**, 5개 region 에서 시작합니다.

![Satya 의 Build 2026 키노트 — Azure HorizonDB Public Preview 발표 슬라이드](build-2026-signals/horizondb-keynote.png){ loading=lazy width="640" }

> 슬라이드 출처: Microsoft Build 2026 keynote · [aka.ms/AzureHorizonDB](https://aka.ms/AzureHorizonDB).

![Azure HorizonDB architecture — disaggregated compute/storage + database-as-a-log (출처: Microsoft Learn)](build-2026-signals/horizondb-architecture.png){ loading=lazy width="560" }

> 아키텍처 다이어그램 출처: [Microsoft Learn · Azure HorizonDB overview](https://learn.microsoft.com/en-us/azure/horizondb/overview).
...
![Satya 의 Build 2026 키노트 — Web IQ 발표 슬라이드 (Best answer quality · Lowest latency · Most token efficient)](build-2026-signals/web-iq-keynote.png){ loading=lazy width="640" }

> 슬라이드 출처: Microsoft Build 2026 keynote · [aka.ms/WebIQ](https://aka.ms/WebIQ).

발표에서 같이 공개된 benchmark 슬라이드는 Microsoft 가 내세운 세 축 — *quality · latency · grounding satisfaction* — 의 실제 수치 비교를 보여줍니다.

![Web IQ benchmarks — quality 79.05, p95 latency 164 ms, grounding satisfaction 우위 (출처: Microsoft Build 2026 keynote)](build-2026-signals/web-iq-benchmarks.png){ loading=lazy width="640" }

> 슬라이드 출처: Microsoft Build 2026 keynote.

![Foundry agent 의 세 구성요소 — model + instructions + tools (출처: Microsoft Learn)](build-2026-signals/foundry-agent-components.png){ loading=lazy width="560" }

> 이미지 출처: [Microsoft Learn · What is Microsoft Foundry Agent Service?](https://learn.microsoft.com/en-us/azure/ai-foundry/agents/overview). Web Search tool 은 이 도구 카탈로그의 일부입니다.
...
Datadog 의 ODSP907 가이드는 명확합니다 — **LETS (Latency · Errors · Traffic · Saturation) 는 유지하되 더 잘게 쪼개고** (Saturation 에 GPU · API rate limit 포함), **Cost · Safety · Quality 세 차원을 새로 추가**. 특히 *Quality* 는 ground truth 가 없는 응답 공간을 — *Hallucination · Relevance · UserSat · Completeness · Retrieval · Coherence* 여섯 메트릭으로 분해합니다.

![Datadog Agent Observability 대시보드 — latency · cost · token 사용량을 한 화면에서 (출처: Datadog Docs)](build-2026-signals/datadog-llm-obs-dashboard.png){ loading=lazy width="640" }

> 이미지 출처: [Datadog Docs · Agent Observability](https://docs.datadoghq.com/llm_observability/). 세션에서 다루는 LETS + Cost/Safety/Quality 체크리스트가 실제 대시보드에 어떻게 투영되는지 참고용.

product-agnostic 체크리스트로 설계되어 Datadog 이 아닌 다른 옵저버빌리티 스택에도 그대로 옮겨 적용할 수 있습니다. 각 메트릭의 측정 방법은 — 세션 노트에.
